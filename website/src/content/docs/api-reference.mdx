# Frimousse API Reference

This file is the package-level API reference for `@slithy/frimousse`.

It covers:

- `EmojiPicker` components
- hooks
- additive supplemental APIs
- custom emoji helpers
- frequency helpers
- search configuration
- styling hooks
- exported types

## Entry Points

### Namespace export

```ts
import { EmojiPicker } from "@slithy/frimousse";
```

Available parts:

- `EmojiPicker.Root`
- `EmojiPicker.Search`
- `EmojiPicker.Viewport`
- `EmojiPicker.List`
- `EmojiPicker.Loading`
- `EmojiPicker.Empty`
- `EmojiPicker.SkinToneSelector`
- `EmojiPicker.ActiveEmoji`
- `EmojiPicker.ActiveSelection`
- `EmojiPicker.SkinTone`

### Named exports

```ts
import {
  buildEmojiPickerFrequentSection,
  createEmojiPickerCustomEmoji,
  createEmojiPickerCustomSection,
  getEmojiPickerUsageKey,
  isEmojiPickerCustomEmoji,
  isNativeSelection,
  isSupplementalSelection,
  rankEmojiPickerUsage,
  recordEmojiPickerUsage,
  useActiveEmoji,
  useActiveSelection,
  useSkinTone,
} from "@slithy/frimousse";
```

## Components

### `EmojiPicker.Root`

Base provider and interaction boundary for the picker tree.

Inherits standard `<div>` props.

Props:

| Prop | Type | Default | Notes |
| --- | --- | --- | --- |
| `onEmojiSelect` | `(emoji: Emoji) => void` | - | Called when a native emoji is selected. |
| `onSelectionChange` | `(selection: EmojiPickerSelection) => void` | - | Called when any picker item is selected, including supplemental items. Additive; does not replace `onEmojiSelect`. |
| `locale` | `Locale` | `"en"` | Emojibase locale. |
| `skinTone` | `SkinTone` | `"none"` | Initial skin tone. |
| `columns` | `number` | `10` | Number of visible list columns. |
| `emojiVersion` | `number` | browser-supported latest | Restricts visible emoji by Emoji version. |
| `emojibaseUrl` | `string` | `"https://cdn.jsdelivr.net/npm/emojibase-data"` | Base URL used to fetch Emojibase JSON datasets. |
| `sticky` | `boolean` | `true` | Makes category headers sticky. |
| `supplemental` | `EmojiPickerSupplementalConfig` | - | Adds consumer-owned sections and supplemental search behavior. |
| `search` | `EmojiPickerSearchConfig` | - | Adds native search enrichment data. |

### `EmojiPicker.Search`

Search input for the picker.

Inherits standard `<input>` props.

Notes:

- Can be used as an uncontrolled input.
- Can also be controlled with `value` and `onChange`.

### `EmojiPicker.Viewport`

Scrollable viewport for loading, empty, and list content.

Inherits standard `<div>` props.

### `EmojiPicker.List`

Virtualized list renderer for categories, rows, and emoji buttons.

Inherits standard `<div>` props.

Props:

| Prop | Type | Default | Notes |
| --- | --- | --- | --- |
| `components` | `Partial<EmojiPickerListComponents>` | - | Replaces internal renderers for category headers, rows, native emoji buttons, and supplemental emoji buttons. |

`EmojiPickerListComponents` shape:

- `CategoryHeader: ComponentType<EmojiPickerListCategoryHeaderProps>`
- `Row: ComponentType<EmojiPickerListRowProps>`
- `Emoji: ComponentType<EmojiPickerListEmojiProps>`
- `SupplementalEmoji: ComponentType<EmojiPickerListSupplementalEmojiProps>`

Renderer prop types:

- `EmojiPickerListCategoryHeaderProps`
  - inherits `<div>` props except `children`
  - `category: Category`
- `EmojiPickerListRowProps`
  - inherits `<div>` props
- `EmojiPickerListEmojiProps`
  - inherits `<button>` props except `children`
  - `emoji: Emoji & { isActive: boolean }`
- `EmojiPickerListSupplementalEmojiProps`
  - inherits `<button>` props except `children`
  - `emoji: SupplementalEmoji & { isActive: boolean }`

### `EmojiPicker.Loading`

Rendered while emoji data is loading.

Inherits standard `<span>` props.

### `EmojiPicker.Empty`

Rendered when the current search has no results.

Inherits standard `<span>` props except `children`.

Props:

| Prop | Type | Notes |
| --- | --- | --- |
| `children` | `ReactNode \| ((props: EmojiPickerEmptyRenderProps) => ReactNode)` | Can be static content or a render callback. |

`EmojiPickerEmptyRenderProps`:

- `search: string`

### `EmojiPicker.SkinToneSelector`

Built-in skin tone toggle button.

Inherits standard `<button>` props except `children`.

Props:

| Prop | Type | Default | Notes |
| --- | --- | --- | --- |
| `emoji` | `string` | `"✋"` | Emoji used as the visual basis for skin tone variations. |

### `EmojiPicker.ActiveEmoji`

Render-prop component for the currently active native emoji.

Props:

| Prop | Type | Notes |
| --- | --- | --- |
| `children` | `(props: EmojiPickerActiveEmojiRenderProps) => ReactNode` | Native-only active item surface. |

`EmojiPickerActiveEmojiRenderProps`:

- `emoji?: Emoji`

### `EmojiPicker.ActiveSelection`

Render-prop component for the currently active selection, including supplemental items.

Props:

| Prop | Type | Notes |
| --- | --- | --- |
| `children` | `(props: EmojiPickerActiveSelectionRenderProps) => ReactNode` | Widened active-item surface. |

`EmojiPickerActiveSelectionRenderProps`:

- `selection?: EmojiPickerSelection`

### `EmojiPicker.SkinTone`

Render-prop component for the current skin tone state.

Props:

| Prop | Type | Default | Notes |
| --- | --- | --- | --- |
| `emoji` | `string` | `"✋"` | Emoji used to derive the variation list. |
| `children` | `(props: EmojiPickerSkinToneRenderProps) => ReactNode` | Render callback. |

`EmojiPickerSkinToneRenderProps`:

- `skinTone: SkinTone`
- `setSkinTone: (skinTone: SkinTone) => void`
- `skinToneVariations: SkinToneVariation[]`

## Hooks

### `useActiveEmoji()`

Returns the currently active native emoji, or `undefined`.

```ts
const activeEmoji = useActiveEmoji();
```

Return type:

- `Emoji | undefined`

Notes:

- Native-only.
- Returns the hovered item or the keyboard-active item.

### `useActiveSelection()`

Returns the currently active selection, including supplemental items.

```ts
const activeSelection = useActiveSelection();
```

Return type:

- `EmojiPickerItemSelection | undefined`

### `useSkinTone(emoji = "✋")`

Returns skin tone state, setter, and skin tone variations for the provided emoji.

```ts
const [skinTone, setSkinTone, skinToneVariations] = useSkinTone();
```

Return type:

- `[SkinTone, (skinTone: SkinTone) => void, SkinToneVariation[]]`

## Additive Supplemental API

### `supplemental` root prop

`EmojiPickerRootProps["supplemental"]` accepts:

```ts
type EmojiPickerSupplementalConfig = {
  sections?: EmojiPickerSection[];
  search?: EmojiPickerSupplementalSearch;
};
```

`EmojiPickerSupplementalSearch`:

| Prop | Type | Default | Notes |
| --- | --- | --- | --- |
| `mode` | `"grouped" \| "unified"` | `"grouped"` | Search result presentation mode for supplemental content. |
| `resultsLabel` | `string` | - | Label used for unified result sections. |

### Items, sections, and selections

```ts
type NativeEmojiPickerItem = {
  kind: "native";
  id: string;
  emoji: string;
  label: string;
};

type SupplementalEmojiPickerItem = {
  kind: "supplemental";
  id: string;
  label: string;
  imageUrl?: string;
  tags?: string[];
  keywords?: string[];
  aliases?: string[];
  data?: unknown;
};

type EmojiPickerItem =
  | NativeEmojiPickerItem
  | SupplementalEmojiPickerItem;

type EmojiPickerSection<TItem extends EmojiPickerItem = EmojiPickerItem> = {
  id: string;
  label?: string;
  position?: "prepend" | "append";
  items: TItem[];
  searchable?: boolean;
};

type NativeEmojiPickerSelection = {
  kind: "native";
  item: NativeEmojiPickerItem;
};

type SupplementalEmojiPickerSelection = {
  kind: "supplemental";
  item: SupplementalEmojiPickerItem;
};

type EmojiPickerSelection =
  | NativeEmojiPickerSelection
  | SupplementalEmojiPickerSelection;
```

### Selection guards

```ts
isNativeSelection(selection)
isSupplementalSelection(selection)
```

Use these to narrow `EmojiPickerSelection` unions safely.

## Custom Emoji Helpers

### `createEmojiPickerCustomEmoji(input)`

Creates a normalized supplemental item with required `id` and `imageUrl`.

Input type:

```ts
type EmojiPickerCustomEmojiInput = {
  id: string;
  label?: string;
  imageUrl: string;
  tags?: string[];
  keywords?: string[];
  aliases?: string[];
  data?: unknown;
};
```

Return type:

- `EmojiPickerCustomEmoji`

### `createEmojiPickerCustomSection(items, options)`

Creates a normalized section of image-backed custom emoji.

Arguments:

- `items: readonly EmojiPickerCustomEmojiInput[]`
- `options: EmojiPickerCustomSectionOptions`

`EmojiPickerCustomSectionOptions`:

```ts
type EmojiPickerCustomSectionOptions = {
  id: string;
  label?: string;
  position?: "prepend" | "append";
  searchable?: boolean;
};
```

Return type:

- `EmojiPickerCustomSection`

### `isEmojiPickerCustomEmoji(item)`

Type guard for a supplemental item with a required `imageUrl`.

## Frequency Helpers

### Types

```ts
type EmojiPickerUsageSource = EmojiPickerItem | EmojiPickerSelection;

type EmojiPickerUsageEntry = {
  key: string;
  item: EmojiPickerItem;
  score: number;
  uses: number;
  lastUsedAt: number;
};

type EmojiPickerUsageOptions = {
  halfLifeMs?: number;
  maxEntries?: number;
  now?: Date | number;
};

type EmojiPickerFrequentSectionOptions = EmojiPickerUsageOptions & {
  id?: string;
  label?: string;
  limit?: number;
  position?: "prepend" | "append";
  searchable?: boolean;
};
```

### `getEmojiPickerUsageKey(source)`

Returns a stable usage key in the form `${kind}:${id}`.

### `rankEmojiPickerUsage(entries, options?)`

Decays and sorts usage entries.

Return type:

- `EmojiPickerUsageEntry[]`

### `recordEmojiPickerUsage(entries, source, options?)`

Adds or updates a usage entry and returns the re-ranked collection.

Return type:

- `EmojiPickerUsageEntry[]`

### `buildEmojiPickerFrequentSection(entries, options?)`

Builds a frequent-items section from ranked usage.

Return type:

- `EmojiPickerFrequentSection | null`

Defaults:

- section id: `"frequently-used"`
- section label: `"Frequently used"`
- section position: `"prepend"`
- section searchable: `false`
- section limit: `20`

## Search Configuration

`EmojiPickerRootProps["search"]` accepts:

```ts
type EmojiPickerNativeSearchConfig = {
  terms?: Record<string, string[]>;
};

type EmojiPickerSearchConfig = {
  native?: EmojiPickerNativeSearchConfig;
};
```

Use this to enrich native emoji matching with consumer-owned search terms.

## Styling Hooks

Stable styling hooks exposed on built-in parts:

- `[frimousse-root]`
- `[frimousse-search]`
- `[frimousse-viewport]`
- `[frimousse-list]`
- `[frimousse-category-header]`
- `[frimousse-row]`
- `[frimousse-emoji]`
- `[frimousse-loading]`
- `[frimousse-empty]`
- `[frimousse-skin-tone-selector]`

Relevant state hook:

- `[data-active]` on emoji buttons

The implementation also uses internal measurement and sizing attributes. Treat those as implementation details unless documented elsewhere.

## Exported Types

### Core

- `Category`
- `Emoji`
- `Locale`
- `SkinTone`
- `SkinToneVariation`

### Items and selections

- `NativeItem`
- `SupplementalItem`
- `SupplementalEmoji`
- `PickerItem`
- `NativeSelection`
- `SupplementalSelection`
- `PickerSelection`
- `EmojiPickerItemSelection`

### Sections and search

- `PickerSection`
- `SupplementalSection`
- `SectionPosition`
- `SupplementalSearch`
- `SearchConfig`
- `EmojiPickerNativeSearchConfig`
- `EmojiPickerSearchConfig`

### Component props and render props

- `EmojiPickerRootProps`
- `EmojiPickerSearchProps`
- `EmojiPickerViewportProps`
- `EmojiPickerListProps`
- `EmojiPickerLoadingProps`
- `EmojiPickerEmptyProps`
- `EmojiPickerEmptyRenderProps`
- `EmojiPickerSkinToneSelectorProps`
- `EmojiPickerActiveEmojiProps`
- `EmojiPickerActiveEmojiRenderProps`
- `EmojiPickerActiveSelectionProps`
- `EmojiPickerActiveSelectionRenderProps`
- `EmojiPickerSkinToneProps`
- `EmojiPickerSkinToneRenderProps`
- `EmojiPickerListComponents`
- `EmojiPickerListCategoryHeaderProps`
- `EmojiPickerListRowProps`
- `EmojiPickerListEmojiProps`
- `EmojiPickerListSupplementalEmojiProps`

### Data types

- `EmojiData`
- `EmojiDataCategory`
- `EmojiDataEmoji`
- `EmojiPickerData`
- `EmojiPickerDataCategory`
- `EmojiPickerDataRow`

### Custom emoji and sections

- `EmojiPickerCustomEmoji`
- `EmojiPickerCustomEmojiInput`
- `EmojiPickerCustomSection`
- `EmojiPickerCustomSectionOptions`
- `CustomEmoji`
- `CustomEmojiInput`
- `CustomSection`
- `CustomSectionOptions`

### Frequency

- `EmojiPickerUsageEntry`
- `EmojiPickerUsageOptions`
- `EmojiPickerUsageSource`
- `EmojiPickerFrequentSection`
- `EmojiPickerFrequentSectionOptions`

### Supplemental additive surface

- `EmojiPickerItem`
- `EmojiPickerSection`
- `EmojiPickerSectionPosition`
- `EmojiPickerSelection`
- `EmojiPickerSupplementalConfig`
- `EmojiPickerSupplementalSearch`
- `NativeEmojiPickerItem`
- `SupplementalEmojiPickerItem`
- `NativeEmojiPickerSelection`
- `SupplementalEmojiPickerSelection`

## Notes

- `useActiveEmoji` and `EmojiPicker.ActiveEmoji` remain native-only.
- `useActiveSelection` and `EmojiPicker.ActiveSelection` include supplemental items.
- `onEmojiSelect` remains native-only.
- `onSelectionChange` is the widened additive callback for both native and supplemental items.
