...
Use row parity plus button position to rotate accent colors across the grid when an item is active. This produces the kind of "colorful button" effect seen in some emoji pickers while still styling only consumer-owned CSS.
...
You can also create a color-wash effect by rendering a blurred duplicate behind the active emoji in a custom `Emoji` component. This keeps the picker logic unchanged and moves the visual treatment into consumer-owned markup and CSS.
