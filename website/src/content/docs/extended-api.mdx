...
`aliases`, `keywords`, and `tags` participate in supplemental search. `imageUrl` is presentation data, and `data` is an opaque consumer payload that the picker carries through without interpreting.
