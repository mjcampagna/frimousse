...
`aliases`, `keywords`, and `tags` participate in supplemental search. `imageUrl` is presentation data, and `data` is an opaque consumer payload that the picker carries through without interpreting. If your app treats shortcode as the canonical identity for custom emoji, use that shortcode as `id` and put alternate forms in `aliases`. If you also want a canonical display-oriented `:shortcode:` token on the item itself, set `shortcode`.
...
If your UI needs one canonical display shortcode, use `getEmojiPrimaryShortcode`. If you need alternate forms too, use `getEmojiShortcodes` with the widened item surface.
