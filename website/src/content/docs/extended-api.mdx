...
Use `onSelectionChange`, `useActiveSelection`, or `EmojiPicker.ActiveSelection` when your UI needs to react to both native and supplemental items.

```tsx
import { EmojiPicker, type EmojiPickerSelection } from "@slithy/frimousse";

function MyEmojiPicker() {
  const handleSelectionChange = (selection: EmojiPickerSelection) => {
  ...
  return (
    <EmojiPicker.Root onSelectionChange={handleSelectionChange}>
      ...
    </EmojiPicker.Root>
  );
}
...
- `useActiveSelection` and `EmojiPicker.ActiveSelection` reflect supplemental items too.
